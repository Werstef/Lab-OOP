...
public:
    ...
    int return_max_student(){
        int maxim = studentsList[0].getNote();
        for(int i = 1; i < studentsNumber; i++){
            if(studentsList[i].getNote() > maxim){
                maxim = studentsList[i].getNote();
            }
        }
        return maxim;
    }

    Student first_5_mark(){
...
int main()
{
    ...
    studentsGroup->showStudentsInGroup();
    cout << studentsGroup->return_max_student() << endl;
    cout << studentsGroup->first_5_mark().getName();
    return 0;
}
